...
describe('retry & timeout', () => {
  ...
  const makeClient = (options?: { timeoutMs?: number; retry?: { retries?: number; baseDelayMs?: number; maxDelayMs?: number; retryOn?: number[] } }): JiraClient =>
  ...
  const mockStatus = (status: number, statusText: string, headers?: Record<string, string>): void => {
    fetchMock.mockResolvedValueOnce({
      ok: false,
      status,
      statusText,
  ...
  it('stops retrying when the signal aborts during the backoff wait', async () => {
    ...
    await expect(jira.searchAll({ jql: 'project = OPS' }, { signal: controller.signal }).next())
