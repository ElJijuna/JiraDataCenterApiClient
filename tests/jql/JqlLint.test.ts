...
import { lintJql, isValidJql } from '../../src/jql/JqlLint';

const errorMessages = (query: string): string[] =>
  lintJql(query).filter((issue) => issue.severity === 'error').map((issue) => issue.message);
...
describe('tokenizeJql', () => {
  ...
  it('handles single quotes and escaped quotes inside strings', () => {
    expect(tokenizeJql("summary ~ 'it\\'s'").map((t) => t.type)).toEqual(['word', 'operator', 'string']);
...
describe('lintJql', () => {
  it('accepts well-formed queries', () => {
    ...
    expect(lintJql('project = "OPS" AND status IN ("Open", "Reopened") ORDER BY updated DESC, priority ASC')).toEqual([]);
  ...
  it('reports unterminated strings', () => {
    expect(errorMessages('summary ~ "abc')).toEqual([expect.stringContaining('Unterminated string')]);
  });
  ...
  it('reports dangling or doubled logical operators', () => {
    expect(errorMessages('AND a = 1')).toEqual([expect.stringContaining('must not start with "AND"')]);
    expect(errorMessages('a = 1 OR')).toEqual([expect.stringContaining('must not end with "OR"')]);
    expect(errorMessages('a = 1 AND OR b = 2')).toContainEqual(expect.stringContaining('"AND" must not be followed by "OR"'));
    expect(errorMessages('a = 1 AND NOT')).toEqual([expect.stringContaining('must not end with "NOT"')]);
  });

  it('reports comparison operators with missing operands', () => {
    expect(errorMessages('= 1')).toContainEqual(expect.stringContaining('missing its left-hand field'));
    expect(errorMessages('a =')).toContainEqual(expect.stringContaining('missing its right-hand value'));
    expect(errorMessages('a = AND b = 2')).toContainEqual(expect.stringContaining('missing its right-hand value'));
    expect(errorMessages('a = = 1')).toContainEqual(expect.stringContaining('Unexpected "=" after comparison operator'));
    expect(errorMessages('a = 1 AND = 2')).toContainEqual(expect.stringContaining('missing its left-hand field'));
  });

  it('reports malformed IN clauses', () => {
    expect(errorMessages('status IN')).toEqual([expect.stringContaining('"IN" must be followed')]);
    expect(errorMessages('status IN Open')).toEqual([expect.stringContaining('parenthesized list or a function call')]);
  ...
  it('reports malformed ORDER BY clauses', () => {
    expect(errorMessages('a = 1 ORDER updated')).toEqual([expect.stringContaining('"ORDER" must be followed by "BY"')]);
    expect(errorMessages('a = 1 ORDER BY')).toEqual([expect.stringContaining('followed by at least one field')]);
    expect(errorMessages('a = 1 ORDER BY updated = 2')).toContainEqual(expect.stringContaining('in ORDER BY clause'));
    expect(errorMessages('ORDER BY created, DESC')).toContainEqual(expect.stringContaining('must follow a field name'));
  });
});
