import { type PaginatedPage, paginate } from '../../src/pagination/paginate';
...
describe('paginate', () => {
  it('walks pages using total until exhausted', async () => {
    const fetchPage = jest.fn(
      (startAt: number, maxResults: number): Promise<PaginatedPage<number>> =>
        Promise.resolve({
  ...
  it('stops when isLast is true even without total', async () => {
    ...
    const fetchPage = jest.fn(
      (): Promise<PaginatedPage<string>> => Promise.resolve(pages.shift() as PaginatedPage<string>),
    );
  ...
  it('stops on a short page when the endpoint reports neither total nor isLast', async () => {
    ...
    const fetchPage = jest.fn(
      (): Promise<PaginatedPage<number>> => Promise.resolve(pages.shift() ?? { items: [] }),
    );
  ...
  it('stops on an empty page', async () => {
    const fetchPage = jest.fn(
      (): Promise<PaginatedPage<number>> => Promise.resolve({ items: [], total: 10 }),
    );
  ...
  it('honors limit, stopping mid-page without fetching further', async () => {
    const fetchPage = jest.fn(
      (startAt: number): Promise<PaginatedPage<number>> =>
        Promise.resolve({
  ...
  it('rejects invalid pageSize and limit', async () => {
    const fetchPage = (): Promise<PaginatedPage<number>> => Promise.resolve({ items: [] });
  ...
  it('throws when the signal is already aborted', async () => {
    ...
    await expect(
      collect(paginate(fetchPage as never, { signal: controller.signal })),
  ...
  it('stops between pages when the signal aborts mid-iteration', async () => {
    const controller = new AbortController();
    const fetchPage = jest.fn((startAt: number): Promise<PaginatedPage<number>> => {
      controller.abort();

      return Promise.resolve({ items: [startAt], total: 10 });
    });

    await expect(
      collect(paginate(fetchPage, { pageSize: 1, signal: controller.signal })),
