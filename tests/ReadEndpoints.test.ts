...
const mockResolution: JiraResolution = {
  self: `${BASE_API}/resolution/1`,
  id: '1',
  name: 'Fixed',
  description: 'Work done.',
};
const mockStatusCategory: JiraStatusCategory = {
  self: `${BASE_API}/statuscategory/3`,
  id: 3,
  key: 'done',
  name: 'Done',
  colorName: 'green',
...
const mockEditMeta: JiraEditMeta = {
  fields: {
    ...
    customfield_10010: {
      ...
      schema: { type: 'any', custom: 'com.pyxis.greenhopper.jira:gh-epic-link', customId: 10010 },
    },
  },
};
  ...
  sections: [
    {
      ...
      id: 'cs',
      issues: [{ key: 'OPS-1', summaryText: 'Connection timeout' }],
    },
  ],
};
const mockAttachment: JiraAttachment = {
  self: `${BASE_API}/attachment/1000`,
  id: '1000',
  filename: 'log.txt',
  created: '2026-07-01T10:00:00.000+0000',
  size: 2048,
  mimeType: 'text/plain',
  content: `${API_URL}/secure/attachment/1000/log.txt`,
};
...
const mockGroups: JiraGroupsPickerResponse = {
  ...
  total: 1,
  groups: [{ name: 'jira-developers', html: '<b>jira-dev</b>elopers' }],
};
...
const mockDashboard: JiraDashboard = {
  id: '10000',
  name: 'System Dashboard',
  self: `${BASE_API}/dashboard/10000`,
};
const mockDashboards: JiraDashboardsResponse = {
  startAt: 0,
  maxResults: 20,
  total: 1,
  dashboards: [mockDashboard],
};
const mockCategory: JiraProjectCategory = {
  self: `${BASE_API}/projectCategory/1`,
  id: '1',
  name: 'Internal',
};
const mockWorkflow: JiraWorkflow = {
  name: 'jira',
  description: 'Default workflow',
  steps: 5,
  default: true,
};
const mockOption: JiraCustomFieldOption = {
  self: `${BASE_API}/customFieldOption/10001`,
  value: 'Blue',
};
const mockColumns: JiraFilterColumn[] = [
  { label: 'Key', value: 'issuekey' },
  { label: 'Summary', value: 'summary' },
];
const mockFilterPermissions: JiraFilterPermission[] = [
  { id: 1, type: 'group', group: { name: 'jira-users' } },
];
const mockEpic: JiraEpic = {
  id: 7,
  key: 'OPS-10',
  self: `${BASE_AGILE}/epic/7`,
  name: 'Checkout revamp',
  summary: 'Revamp checkout',
  color: { key: 'color_2' },
  done: false,
};
...
const mockVersions: PagedResponse<JiraVersion> = {
  startAt: 0,
  maxResults: 50,
  total: 1,
  isLast: true,
  values: [{ id: '20001', name: '1.1.0', released: false, projectId: 1 }],
};
const mockBoardProjects: PagedResponse<JiraBoardProject> = {
  startAt: 0,
  maxResults: 50,
  total: 1,
  isLast: true,
  values: [{ id: '1', key: 'OPS', name: 'Operations' } as unknown as JiraBoardProject],
};
const mockEpics: PagedResponse<JiraEpic> = {
  startAt: 0,
  maxResults: 50,
  total: 1,
  isLast: true,
...
describe('JiraClient read endpoints (P0 batch)', () => {
  ...
  describe('metadata', () => {
    ...
    it('fetches a custom field option', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        `${BASE_API}/customFieldOption/10001`,
        expect.any(Object),
      );
    });
  });
  ...
  describe('groups', () => {
    ...
    it('fetches group members', async () => {
      ...
      const members = await jira.groupMembers({
  ...
  describe('issue picker', () => {
    it('fetches issue suggestions', async () => {
      mockJson(mockPicker);
      const result = await jira.issuePicker({
        query: 'timeout',
        currentJQL: 'project = OPS',
        showSubTasks: true,
      });
  ...
  describe('filters', () => {
    it('fetches filter columns and permissions', async () => {
      ...
      expect(fetchMock).toHaveBeenLastCalledWith(
        `${BASE_API}/filter/10000/permission`,
        expect.any(Object),
      );
    });
  });
  ...
  describe('issue & project metadata resources', () => {
    ...
    it('fetches createmeta fields for a project and issue type', async () => {
      ...
      expect(fetchMock).toHaveBeenCalledWith(
        `${BASE_API}/issue/createmeta/OPS/issuetypes/1`,
        expect.any(Object),
      );
